...
- Proxy must terminate QUIC from clients and present a valid client cert when connecting to backends.
...
- On QUIC connection: create ClientSession, attach correlation id, track client certificate.
...
Open items
- Confirm backend trust model for proxy client certs (mTLS trust root).
- Verify stream behavior (single bidirectional vs multi-stream) for gameplay and asset transfer.
